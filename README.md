...
Replace the default cert.pem with your organization's CA-signed certificates for production use. OR if you don't have any CA signed cert run the below command in your command line CrypTrus2.0 directory to generate a self-signed certificate for SSL/TLS Encryption even though your browser will flag the certificate just advance because the Certificate Authority can't verify the certificate but the encryption will still work.
```bash
pkg install openssl
